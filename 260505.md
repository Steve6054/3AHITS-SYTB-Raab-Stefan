...
then
    ...
    nl $FILE > tmp.tmp && mv tmp.tmp $FILE
    exit
else
    STDIN="/dev/stdin"
    nl $STDIN >> number.txt
...
Damit `nl` die Datei selbst ändern kann muss das Ergebnis erst temporär gespeichert werden, weil `nl` selbst keine Dateien verändern kann.

Test: **Keine Zeit mehr gehabt!**