...
curl -s -o "$TEMPDIR/sha512sums" https://the.earth.li/~sgtatham/putty/0.83/sha512sums
grep "w64/putty.zip" "$TEMPDIR/sha512sums" > "$TEMPDIR/sha512line"

CHECKSUM=$(cut -d' ' -f1 "$TEMPDIR/sha512line")

echo "Herunterladen des .zip"
curl -s -O https://the.earth.li/~sgtatham/putty/0.83/w64/putty.zip
...
Bemerkung:
Meine Version des Script ist festgelegt auf diese eine Version von putty. Wie man die neueste Version herunterladen und prüfen kann, habe ich nicht herausgefunden, da man über Links mit `latest` wieder ein File bekommt in dem ein Link auf das eigentliche File ist.
