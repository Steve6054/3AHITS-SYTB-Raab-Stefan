...
[ -e $DEST ] && {
    [ $SRC -nt $DEST ] && pandoc "$SRC" -o "$DEST" #File existiert aber ist veraltet
} || pandoc "$SRC" -o "$DEST" #File existiert noch nicht
```

ACHTUNG: `pandoc` war noch nicht installiert. Mit `sudo apt install` installieren!
...
```bash
┌──(kali㉿kali)-[~/260428]
└─$ ./ConvertToHTML.sh Test.md Test.html
```
